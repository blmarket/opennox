...
func nox_gui_getWindowOffs_46AA20(win *nox_window, px, py *C.uint32_t) C.int32_t {
	if win == nil {
		*px = 0
		*py = 0
		return -2
	}
	pos := asWindow(win).Offs()
	*px = C.uint32_t(pos.X)
	*py = C.uint32_t(pos.Y)
	return 0
}
...
func nox_xxx_wndPointInWnd_46AAB0(p *C.uint32_t, x, y C.int32_t) C.bool {
	win := AsWindowP(unsafe.Pointer(p))
	pos := win.GlobalPos()
	sz := win.Size()
	px, py := int32(x), int32(y)
	left, top := int32(pos.X), int32(pos.Y)
	right, bottom := left+int32(sz.X), top+int32(sz.Y)
	return C.bool(px >= left && px <= right && py >= top && py <= bottom)
}
...
func nox_xxx_wndGetFlags_46ADA0(win *nox_window) C.int32_t {
	if win == nil {
		return -2
	}
	flags := C.uint32_t(asWindow(win).Flags)
