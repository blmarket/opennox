...
func makeUppercase(filename, functionName string) error {
	...
	funcRegex := regexp.MustCompile(`^func\s+\(m\s+\*Struct88Module\)\s+` + regexp.QuoteMeta(functionName) + `\s*\(`)
...
func generateExportFunction(funcInfo *FunctionInfo) []string {
	...
	if funcInfo.HasReturn {
		...
		lines = append(lines, fmt.Sprintf("\treturn Struct88Module.%s(%s)", upperFuncName, argStr))
	} else {
		funcDecl = fmt.Sprintf("func %s(%s) {", funcInfo.Name, paramStr)
		lines = append(lines, funcDecl)
		lines = append(lines, fmt.Sprintf("\tStruct88Module.%s(%s)", upperFuncName, argStr))
	}
