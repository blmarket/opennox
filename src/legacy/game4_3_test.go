...
func TestGame43SafeUpdateAndTilePaths(t *testing.T) {
	h := newGameLogicHarness(t)
	h.loadBlobData()
	h.setServer(0, 30)
	got := C_game43SafeUpdatePaths()
	require.Zero(t, got.emptyWeapon)
	require.Equal(t, uint8(0x40), got.toggleOn)
	require.Zero(t, got.toggleOff&0x40)
	require.Equal(t, uint32(1), got.deadline)
	require.Zero(t, got.teleportEmpty)
	require.Zero(t, got.rechargeMissing)
	require.Equal(t, 1, got.rechargeChanged)
	require.Zero(t, got.rechargeUnchanged)
	require.Equal(t, uint8(5), got.charge)
	require.Equal(t, uint8(50), got.percent)
	require.Zero(t, got.rechargeRateNil)
	require.Equal(t, uint32(2), got.barrelNext)
	require.Equal(t, uint32(3), got.burnNext)
	require.Equal(t, uint32(1), got.toxicRemain)
	require.Equal(t, "ERROR_NAME_TOO_LONG!", got.longName)
	require.Equal(t, "ERROR_NAME_TOO_LONG!", got.shortName)
	require.Equal(t, 1, got.tileDisabled)
	require.Equal(t, 1, got.tileDelete)
	require.Equal(t, -1, got.tileMissing)
	require.Equal(t, 1, got.tileNone)
	require.Zero(t, got.tileInvalid)
	require.Equal(t, 1, got.tileSubtileWithoutType)
	require.Zero(t, got.nearbyItem)
	require.Equal(t, 1, got.lesserStaff)
	require.Equal(t, 1, got.readGuard)
}
